...
class DefaultPublisherTests: XCTestCase {
    ...
    func testTrack_success() {
        ...
        publisher.track(trackable: trackable) { result in
            ...
            case .success:
                expectation.fulfill()
            case .failure:
                XCTFail("onError callback shouldn't be called")
            }
        }
    ...
    func testTrack_error_duplicate_track() {
        ...
        publisher.track(trackable: Trackable(id: "DuplicateTrackableId")) { result in
            switch result {
            case .success:
                XCTFail("onSuccess callback shouldn't be called")
    ...
    func testAdd_success_thread() {
        ...
        // Result callback should be called on main thread
        // Notice - in case of failure it will crash whole test suite
        publisher.add(trackable: trackable) { result in
            switch result {
            case .success:
    ...
    func testAdd_error_thread() {
        ablyService.trackCompletionHandler = { result in
            result?(.failure(AssetTrackingError.publisherError("TestError")))
        }
    ...
    func testRemove_success() {
        ...
        ablyService.stopTrackingCompletionHandler = { result in
