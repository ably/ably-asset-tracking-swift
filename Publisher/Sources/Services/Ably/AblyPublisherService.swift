...
/**
 Public wrapper for ARTRealtimeConnectionState, as we don't want to pass Ably classes to our clients
 */
...
class AblyPublisherService {
    ...
    private func setup() {
        ...
        client.connection.on { [weak self] stateChange in
            ...
            strongSelf.delegate?.publisherService(sender: strongSelf,
    ...
    func track(trackable: Trackable, completion: ((Error?) -> Void)?) {
        guard channel == nil
            ...
            // TODO: Throw an AblyError when GeoJSON branch is merged
            return
        }
    ...
    private func sendAssetLocation(location: CLLocation, withName name: EventName, completion: ((Error?) -> Void)?) {
        ...
        guard let data = try? JSONEncoder().encode([geoJSON]) else {
